...
// Helper function to upload media to media worker
async function uploadMediaToWorker(
  mediaUrl: string,
  ...
  try {
    const response = await mediaWorker.fetch(
      "https://fake-host/", // URL is ignored for service bindings
      {
        method: "POST",
        headers: {
    ...
    if (!response.ok) {
      console.error(`Failed to upload ${mediaType}:`, await response.text());
      return null;
    }
...
// Function to process media uploads for processed blocks
async function processMediaUploads(
  ...
  const updatedBlocks: ProcessedBlock[] = [];

  for (const block of processedBlocks) {
    let updatedBlock = { ...block };

    // Process media blocks (image and video)
    if ((block.type === "image" || block.type === "video") && block.content) {
      const mediaUrl = block.content;
      ...
      const uploadedUrl = await uploadMediaToWorker(
        mediaUrl,
      ...
      if (uploadedUrl) {
        ...
        updatedBlock.content = uploadedUrl;

        // Update the HTML with the new URL
        if (block.type === "image") {
          updatedBlock.html = updatedBlock.html.replace(mediaUrl, uploadedUrl);
        } else if (block.type === "video") {
          updatedBlock.html = updatedBlock.html.replace(mediaUrl, uploadedUrl);
        }
      }
    ...
    if (block.children && block.children.length > 0) {
      updatedBlock.children = await processMediaUploads(
        block.children,
        userId,
        secretKey,
        mediaWorker
    ...
    try {
      ...
      try {
        ...
        return new Response(
          JSON.stringify({
            success: false,
            error: "Failed to process Notion page",
          }),
          {
            status: 500,
            headers: { "Content-Type": "application/json" },
